...
Type
  ...
  TSimpleRTTI<T : class, constructor> = class(TInterfacedObject, iSimpleRTTI<T>)
    ...
    public
      ...
      function DataBaseType ( var aDataBaseType : String) : ISimpleRTTI<T>;
      function GeneratorName ( var aGeneratorName : String) : ISimpleRTTI<T>;
...
function TSimpleRTTI<T>.DataBaseType(var aDataBaseType: String): ISimpleRTTI<T>;
var
  vInfo   : PTypeInfo;
...
begin
  ...
  try
    ...
    if vTypRtti.Tem<Database> then
      aDataBaseType := vTypRtti.GetAttribute<DataBase>.Database;
  finally
    vCtxRtti.Free;
  end;
          ...
          begin
            ...
            begin
              case prpRtti.PropertyType.TypeKind of
                ...
                tkFloat:
                begin
                  if Pos('Z', Field.AsString) <= 0 then
                    Value := Field.AsFloat;
                end;
...
function TSimpleRTTI<T>.GeneratorName(
  var aGeneratorName: String): ISimpleRTTI<T>;
var
  vInfo   : PTypeInfo;
  vCtxRtti: TRttiContext;
  vTypRtti: TRttiType;
begin
  ...
  try
    vTypRtti := vCtxRtti.GetType(vInfo);
    if vTypRtti.Tem<Generator> then
      aGeneratorName := vTypRtti.GetAttribute<Generator>.Generator;
  finally
    vCtxRtti.Free;
  end;
end;
